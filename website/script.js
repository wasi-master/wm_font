document.addEventListener("DOMContentLoaded", () => {
  ...
  if (
    ...
    const renderGlyphs = () => {
      ...
      const itemsToRender =
        isExpanded || !showButton ? filtered : filtered.slice(0, LIMIT);
    ...
    glyphGrid.addEventListener("click", (e) => {
      ...
      navigator.clipboard.writeText(
        card.querySelector(".glyph-char").textContent,
      );
  ...
  if (imageModal && modalImg) {
    ...
    document
      .querySelectorAll(".use-case-image, .drawing-sheet-image")
      .forEach((picture) => {
  ...
  const refreshColorSelector = () => {
    ...
    if (colorPickerContainer) {
      colorPickerContainer.innerHTML = activePalette
        .map((color, index) => {
  ...
  if (pgEditable) {
    ...
    if (pgResetBtn) {
      pgResetBtn.addEventListener("click", () => {
        ...
        if (pgSize) {
          pgSize.value = pgDefaults.size;
          pgSizeVal.textContent = `${pgDefaults.size}px`;
        }
        if (pgHeight) {
          pgHeight.value = pgDefaults.height;
          pgHeightVal.textContent = pgDefaults.height;
        }
        if (pgSpacing) {
          pgSpacing.value = pgDefaults.spacing;
          pgSpacingVal.textContent = `${pgDefaults.spacing}px`;
        }
        if (pgSample) {
          pgSample.value = "default";
        }
  ...
  if (navToggle && navMenu) {
    ...
    // Close mobile navigation menu and highlight the currently active tab on click
  ...
  comicDownloadBtn.addEventListener("click", () => {
    ...
    img.onload = () => {
      ...
      const { multiplier, minPx } = getBubbleFontSettings();
      const baseFontPx = comic.naturalW * (comic.fontScale ?? 0.009);
    ...
    img.onerror = () => {
      alert(
        "Could not load the comic image for download. Make sure the page is served from a local server.",
      );
    };
  });
